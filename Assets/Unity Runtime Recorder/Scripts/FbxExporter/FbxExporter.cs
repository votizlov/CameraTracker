...
public class FbxExporter : MonoBehaviour
{
    ...
    // objs to track
    [SerializeField] Transform[] observeTargets;
    ...
    void Start()
    {
        sourceFilePath = Application.streamingAssetsPath + "/SourceCamera/camera.fbx";
        exportFilePath = Application.persistentDataPath + "/test.fbx";
        SetupRecordItems();
        if (!(PlayerPrefs.GetInt("isTemplateFileLoaded") > 0))
        {
            PlayerPrefs.SetInt("isTemplateFileLoaded",1);
            StartCoroutine(CopyTemplate());
        }
    }
    
    IEnumerator CopyTemplate()
    {
        string fromPath = Application.streamingAssetsPath +"/SourceCamera/";
        //In Android = "jar:file://" + Application.dataPath + "!/assets/" 
        string toPath =   Application.persistentDataPath +"/";

        string[] filesNamesToCopy = new string[] { "camera.fbx" };
    ...
    private IEnumerator RecordAnim()
    {
        if (isRecording)
        {
            for (int i = 0; i < trackers.Length; i++)
    ...
    IEnumerator ExportToFile()
    {
        ...
        for (int i = 0; i < observeTargets.Length; i++)
        {
            ...
            fbxObj.AddAnimationCurveNode(animCurveNodeT_id, FbxAnimationCurveNodeType.Translation,
                ExportHelper.UnityToMayaPosition(observeTargets[i].localPosition));
            fbxObj.AddAnimationCurveNode(animCurveNodeR_id, FbxAnimationCurveNodeType.Rotation,
                ExportHelper.UnityToMayaRotation(observeTargets[i].localRotation));
            fbxObj.AddAnimationCurveNode(animCurveNodeS_id, FbxAnimationCurveNodeType.Scale,
            ...
            // setup connections
            fbxConn.AddConnectionItem("AnimCurveNode", "T", animCurveNodeT_id, "Model", objName, objId, "OP",
                "Lcl Translation");
            fbxConn.AddConnectionItem("AnimCurveNode", "R", animCurveNodeR_id, "Model", objName, objId, "OP",
                "Lcl Rotation");
            fbxConn.AddConnectionItem("AnimCurveNode", "S", animCurveNodeS_id, "Model", objName, objId, "OP",
                "Lcl Scaling");

            fbxConn.AddConnectionItem("AnimCurveNode", "T", animCurveNodeT_id, "AnimLayer", "BaseLayer",
                animBaseLayerId, "OO", "");
            fbxConn.AddConnectionItem("AnimCurveNode", "R", animCurveNodeR_id, "AnimLayer", "BaseLayer",
                animBaseLayerId, "OO", "");
            fbxConn.AddConnectionItem("AnimCurveNode", "S", animCurveNodeS_id, "AnimLayer", "BaseLayer",
                animBaseLayerId, "OO", "");

            fbxConn.AddConnectionItem("AnimCurve", "", curveT_X_id, "AnimCurveNode", "T", animCurveNodeT_id, "OP",
                "d|X");
            fbxConn.AddConnectionItem("AnimCurve", "", curveT_Y_id, "AnimCurveNode", "T", animCurveNodeT_id, "OP",
                "d|Y");
            fbxConn.AddConnectionItem("AnimCurve", "", curveT_Z_id, "AnimCurveNode", "T", animCurveNodeT_id, "OP",
                "d|Z");

            fbxConn.AddConnectionItem("AnimCurve", "", curveR_X_id, "AnimCurveNode", "R", animCurveNodeR_id, "OP",
                "d|X");
            fbxConn.AddConnectionItem("AnimCurve", "", curveR_Y_id, "AnimCurveNode", "R", animCurveNodeR_id, "OP",
                "d|Y");
            fbxConn.AddConnectionItem("AnimCurve", "", curveR_Z_id, "AnimCurveNode", "R", animCurveNodeR_id, "OP",
                "d|Z");

            fbxConn.AddConnectionItem("AnimCurve", "", curveS_X_id, "AnimCurveNode", "S", animCurveNodeS_id, "OP",
                "d|X");
            fbxConn.AddConnectionItem("AnimCurve", "", curveS_Y_id, "AnimCurveNode", "S", animCurveNodeS_id, "OP",
                "d|Y");
            fbxConn.AddConnectionItem("AnimCurve", "", curveS_Z_id, "AnimCurveNode", "S", animCurveNodeS_id, "OP",
                "d|Z");
